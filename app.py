...
# 2. AI 모델 설정 (에러 방지를 위해 가장 보편적인 모델명 사용)
try:
    if "GOOGLE_API_KEY" in st.secrets:
        genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
        # 'models/gemini-1.5-flash'는 현재 유료/무료 티어 모두에서 가장 안정적인 이름입니다.
        model = genai.GenerativeModel('models/gemini-1.5-flash')
    else:
        st.error("⚠️ API 키가 감지되지 않네! Secrets 설정을 다시 확인해주게.")
        ...
        if st.button("평생 운명 확인"):
            if n_in:
                with st.spinner("사주 단자를 보는 중..."):
                    res = model.generate_content(f"이름:{n_in}, 생일:{b_in}, 시간:{t_in}. 평생 사주와 성격, 운명을 노인 말투로 자세히 풀어줘.")
                    st.write(res.text)
